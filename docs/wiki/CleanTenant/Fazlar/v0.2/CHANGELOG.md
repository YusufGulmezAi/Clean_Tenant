...
## v0.2.12 — 2026-05-21 — Kullanıcı Yönetimi Revizyonu

**Oturum notu:** [[v0.2.12-oturum]]

### Kapsam
İlk giriş şifre zorlaması, OTP şifre sıfırlama, kullanıcı onboarding wizard, scope-bazlı aktif/pasif kontrolü ve tenant kapsamlı kullanıcı görünümü.

### Eklenen Özellikler

| Özellik | Açıklama |
|---|---|
| `RequiresPasswordChange` | Yeni kullanıcılar ilk girişte şifre değiştirmek zorunda |
| `IVerificationCodeService` | Redis-backed 6 haneli OTP; single-use |
| Şifre sıfırlama akışı | `/forgot-password` + `/reset-password` sayfaları, enumeration korumalı |
| `UserOnboardingWizard` | Debounced arama + mevcut kullanıcı atama + yeni kullanıcı oluşturma |
| `DeactivateTenantUserCommand` | `UserRoleAssignment.IsActive = false` — global ban değil |
| `ReactivateTenantUserCommand` | Pasif atamaları tekrar aktifleştirir |
| Tenant kapsamlı kullanıcı görünümü | Tenant + Company scope atamalar birleşik listede |
| NavMenu yeniden yapılanması | Context-sensitive görünüm, permission gating |

### Mimari Not: `User.IsActive` vs `UserRoleAssignment.IsActive`

- `User.IsActive = false` → global ban, tüm scope'larda erişim kesiyor
- `UserRoleAssignment.IsActive = false` → yalnız o scope'ta erişim kesiyor
- Tenant admin yalnız kendi kapsamını yönetir; kullanıcı diğer tenant'larında etkilenmez
