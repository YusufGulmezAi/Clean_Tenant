---
name: Proje Mevcut Durumu — v0.2.10 tamam, sıradaki v0.3
description: v0.2.10 Lokalizasyon (admin sayfası dahil) commit edildi (tag v0.2.10); sıradaki v0.3 Unit/Resident
type: project
...
Plan dosyası: `C:\Users\yusuf\.claude\plans\soft-doodling-sutton.md` (v0.3 detayı henüz yazılmadı)

**Kapsam:**
- **Domain**: `Unit`, `Resident`, `UnitResident` entity'leri (Main DB)
- **Application**: Unit/Resident CRUD handler'ları (Create/Get/Update/Delete/AssignResident)
- **WebApi**: `UnitEndpoints`, `ResidentEndpoints`
- **ManagementApp**: `CompanyArea/Units` listesi + Resident yönetimi
- **PortalApp**: Sakin self-servis (kendi birim bilgisi)

NavMenu'de "Birimler" / "Sakinler" linkleri zaten "Faz 1.6" / "Faz 1.7" chip'leriyle placeholder olarak duruyor; v0.3'te aktive edilir.
